...
class DISTS(torch.nn.Module):
    def __init__(self, load_weights=True):
        ...
        if load_weights:
            weights = torch.load('./utils/DISTS/weights.pt')
...
def compute_DISTS(ref, deg):
    model = DISTS().to(ref.device)
    return model(ref.unsqueeze(0).unsqueeze(0), deg.unsqueeze(0).unsqueeze(0))
